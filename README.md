# Ghost + Traefik + Let's Encrypt on Docker Compose
...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14865/badge)](https://www.bestpractices.dev/projects/14865)
...
Every release from v1.8.9 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `ghost-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `ghost-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `ghost-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob ghost-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle ghost-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/ghost-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact ghost-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path ghost-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/ghost-traefik-letsencrypt-docker-compose
```
...
**Restore** with the interactive scripts (`chmod +x *.sh` once): `./ghost-restore-database.sh`, then `./ghost-restore-application-data.sh`. Each lists the backups and asks, or takes a file name as its argument; both read every path and credential from the running backups container, and CI runs both on every push.
