...
db_restore() {
  backups_sh "mysql -h $DB_HOST -u $DB_USER -p\"$DB_PASS\" -e 'DROP DATABASE \`$DB_NAME\`; CREATE DATABASE \`$DB_NAME\`;' \
    && gunzip -c $1 | mysql -h $DB_HOST -u $DB_USER -p\"$DB_PASS\" $DB_NAME" 2>/dev/null
}
