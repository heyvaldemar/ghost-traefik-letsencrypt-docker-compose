#!/usr/bin/env bash
# ghost-restore-application-data.sh [backup-file-name]
#
# Replaces Ghost's application data with one of the archives the backups
# service wrote.
#
#   ./ghost-restore-application-data.sh               list and ask
#   ./ghost-restore-application-data.sh <file-name>   restore that one
#
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Ghost." >&2
  exit 1
fi
